...
export function buildAgentSystemPrompt(input: AgentSystemPromptInput): string {
  ...
  return [
    ...
    '2.1 有 2-8 个相互独立、无副作用且可并发的只读观察工具调用时，可一次输出：{"action":"tools","tools":[{"tool":"工具名","input":{参数},"thought":"原因"}],"thought":"批量原因"}。写入、Shell、网络和高风险工具必须使用单个 tool 动作逐轮调用，以便每项操作拥有独立的权限、检查点和恢复边界。',
    '3. 已能回答用户时输出：{"action":"final","completionClaim":"completed|partial|blocked|historical","answer":"给用户的最终中文回答"}。计划模式必须额外提供结构化 plan 字段；副作用未完成时必须使用 partial/blocked，引用旧状态时使用 historical。',
    "4. 有依赖关系或任何副作用的工具必须逐轮调用；只有相互独立、纯只读且明确支持并发的工具才能使用 tools 批量动作。禁止把后一步依赖前一步结果的调用放进同一批。",
    ...
    "13. 读取代码先搜索或定位行号，再用 read_file(path,startLine,lineCount) 分段读取；大文件禁止无范围整文件读取。返回 nextStartLine 时从该位置继续，单行压缩文件改用 byteOffset+maxBytes。",
    "14. 修改已有文件优先使用 apply_patch，并传入最近一次 read_file 返回的 expectedSha256。write_file 只用于新文件或小文件完整替换；哈希冲突时重新读取目标范围，禁止盲目覆盖。",
    input.systemHint,
